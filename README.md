...
# 3. Execute as migrations (cria o banco automaticamente no primeiro run)
dotnet run
```

A API ficará disponível em `http://localhost:5215`.  
A documentação Swagger estará em `http://localhost:5215/swagger`.
...
> **Atenção:** A URL da API está definida em `lib/Core/Services/api_service.dart`.  
> Ajuste o `baseUrl` de acordo com o ambiente:
>
> | Ambiente | URL |
> |---|---|
> | Flutter Web / Windows | `http://localhost:5215` |
> | Android Emulator | `http://10.0.2.2:5215` |
> | Dispositivo físico | `http://<IP_DA_MÁQUINA>:5215` |
